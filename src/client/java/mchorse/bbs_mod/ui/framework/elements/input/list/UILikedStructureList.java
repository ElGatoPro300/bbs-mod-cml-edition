...
/**
 * List used specifically to render liked structures with missing asset detection.
 */
public class UILikedStructureList extends UIList<StructureLikeManager.LikedStructure>
{
    private final UIIcon unlikeIcon;
    private Consumer<StructureLikeManager.LikedStructure> unlikeCallback;

    public UILikedStructureList(Consumer<List<StructureLikeManager.LikedStructure>> callback)
    {
        super(callback);
    ...
    public void setStructures(List<StructureLikeManager.LikedStructure> structures)
    {
        this.list.clear();

        if (structures != null)
        {
            this.list.addAll(structures);
        }

        this.update();
    }

    public List<StructureLikeManager.LikedStructure> getStructures()
    {
        return Collections.unmodifiableList(this.list);
    }

    public void setUnlikeCallback(Consumer<StructureLikeManager.LikedStructure> callback)
    {
        this.unlikeCallback = callback;
    }

    @Override
    protected void renderElementPart(UIContext context, StructureLikeManager.LikedStructure element, int i, int x, int y, boolean hover, boolean selected)
    {
        if (element == null)
        {
            return;
        ...
        {
            display = this.truncateText(context, display, maxWidth);
        }
    ...
    public boolean subMouseClicked(UIContext context)
    {
        if (this.area.isInside(context) && context.mouseButton == 0)
        {
            ...
            StructureLikeManager.LikedStructure element = this.getElementAt(index);
